...
import './style.css'
import {getSessionId,loadMockHistory,saveMockHistory} from './mockChat'
...
function App(){
 ...
 const refresh=async(id:number)=>{const data=await request<Overview>(`/datasets/${id}/overview`);if(ONLINE_MOCK&&current!==null&&current!==id)setMessages([]);setOverview(data);setCurrent(id);setDatasets(await request<Dataset[]>('/datasets'));setTimeout(()=>document.getElementById('insights')?.scrollIntoView({behavior:'smooth'}),80)}
 ...
 const upload=async(file:File)=>{setError('');setBusy('reading');try{const form=new FormData();form.append('file',file);const p=await request<Preview>('/preview',{method:'POST',body:form});setPreview(p);setMapping(p.suggested_mapping);if(ONLINE_MOCK)setNotice('演示导入：未读取所选文件，将使用内置虚构反馈。')}catch(e){setError((e as Error).message)}finally{setBusy('')}}
 const importRows=async(p:Preview,source:'upload'|'paste'='upload')=>{setBusy('analyzing');setError('');try{const result=await request<{dataset_id:number;imported:number;skipped:number;duplicate:number;analysis_errors:unknown[]}>('/import',json({name:p.name,headers:p.headers,rows:p.rows,mapping,source}));setPreview(null);setPasteOpen(false);setPaste('');await refresh(result.dataset_id);setNotice(`${ONLINE_MOCK?'演示模式 · ':''}导入 ${result.imported} 条 · 跳过 ${result.skipped} 条 · 重复 ${result.duplicate} 条${result.analysis_errors.length?' · 部分模型分析失败，已回退模拟分类':''}`)}catch(e){setError((e as Error).message)}finally{setBusy('')}}
 ...
 const ask=async(q=question)=>{if(!q.trim()||!current||chatBusy)return;setChatError('');setChatBusy(true);const message=q.trim();setMessages(m=>[...m,{role:'user',text:message}]);setQuestion('');setContextLabel('');try{const r=await request<{mode:string;answer:string}>(`/datasets/${current}/chat`,json({question:message}));setMessages(m=>[...m,{role:'agent',text:r.answer,mode:r.mode}])}catch(e){setChatError((e as Error).message)}finally{setChatBusy(false)}}
 ...
 const drawerCount=drawer?.type==='cluster'?detail?.count||0:drawer?.type==='evidence'?1:drawer?.metric==='themes'?overview?.issues.length||0:drawer?.metric==='negative'?overview?.negative||0:drawer?.metric==='priority'?overview?.issues[0]?.count||0:overview?.total||0
 const drawerShare=drawer?.type==='cluster'?'占当前数据 '+(detail?.share||0)+'%':drawer?.metric==='negative'?'占当前数据 '+(overview?.negative_share||0)+'%':'当前数据集'
 const observed=drawer?.type==='cluster'?'当前 '+(detail?.denominator||0)+' 条反馈中，'+(detail?.count||0)+' 条归入「'+drawerTitle+'」。点击下方编号可核对原文。':drawer?.type==='evidence'?(ONLINE_MOCK?'以下是浏览器演示数据保留的虚构反馈与来源行号。':'以下是数据库保留的原始反馈与来源行号。'):drawer?.metric==='themes'?'当前识别 '+(overview?.issues.length||0)+' 个主题。':drawer?.metric==='negative'?'负向反馈 '+(overview?.negative||0)+' 条；分母是 '+(overview?.total||0)+' 条有效反馈。':drawer?.metric==='priority'?'「'+(overview?.issues[0]?.name||'')+'」提及 '+(overview?.issues[0]?.count||0)+' 条，是当前频次最高的主题。':'当前数据集包含 '+(overview?.total||0)+' 条有效反馈。'
   ...
   <section className="wrap work" id="conversation">
    <div>
     ...
     <div className={'drop'+(drag?' drag':'')} onDragOver={e=>{e.preventDefault();setDrag(true)}} onDragLeave={()=>setDrag(false)} onDrop={e=>{e.preventDefault();setDrag(false);const file=e.dataTransfer.files?.[0];if(file)upload(file)}}>
      ...
      <div className="upload-copy"><strong>放入你的用户反馈</strong><small>{ONLINE_MOCK?'演示模式 · 选文件后使用虚构样例，不读取文件内容':'CSV、Excel 或 TXT · 拖放文件至此'}</small></div>
    ...
    <div className="chat">
     ...
     <div className="messages" aria-live="polite">
      {messages.length===0&&<div className="message agent"><span className="who">FOLIO</span>{ONLINE_MOCK?'你好。已载入虚构反馈后，可以问我 Top Issues、原始证据、版本和时间分布。所有结论仅用于演示。':'你好。上传反馈，或先载入示例数据。你也可以直接问我想了解什么。'}</div>}
      ...
      {chatBusy&&<div className="message agent"><span className="who">FOLIO</span>{ONLINE_MOCK?'正在查询演示数据…':'正在查询当前数据集…'}</div>}
